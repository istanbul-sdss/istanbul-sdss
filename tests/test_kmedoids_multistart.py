"""
Sprint 2 #14 + #20: K-Medoids multi-start + stable seed regresyonu.
...
def test_n_restarts_default_is_one():
    ...
    od, b, t = _make_data()
    r = coz(od, b, t, p=3, solver="kmedoids")
    # Hâlâ K-Medoids dönmeli, convergence raporlanmalı
    assert r.yontem == "K-Medoids"
    assert r.kmedoids_converged in (True, False)
    assert isinstance(r.kmedoids_iterations, int)
...
def test_multistart_returns_no_worse_cost():
    ...
    single = coz(od, b, t, p=3, solver="kmedoids", n_restarts=1)
    multi  = coz(od, b, t, p=3, solver="kmedoids", n_restarts=5, random_state=42)
...
def test_seed_reproducibility():
    ...
    r1 = coz(od, b, t, p=3, solver="kmedoids", n_restarts=5, random_state=123)
    r2 = coz(od, b, t, p=3, solver="kmedoids", n_restarts=5, random_state=123)
...
def test_different_seeds_can_diverge():
    ...
    """
    od, b, t = _make_data(n_bina=30, n_alan=10)
    r_a = coz(od, b, t, p=3, solver="kmedoids", n_restarts=5, random_state=0)
    r_b = coz(od, b, t, p=3, solver="kmedoids", n_restarts=5, random_state=999)
    # Her ikisi de geçerli K-Med sonucu
    assert r_a.yontem == r_b.yontem == "K-Medoids"
...
def test_multistart_with_capacity_works():
    ...
    r = coz(
        od, b, t, p=3, solver="kmedoids", kapasite=True,
...
def test_random_state_none_still_runs():
    ...
    r = coz(od, b, t, p=2, solver="kmedoids", n_restarts=3, random_state=None)
    assert r.yontem == "K-Medoids"
...
def test_n_restarts_zero_treated_as_one():
    ...
    r = coz(od, b, t, p=2, solver="kmedoids", n_restarts=0)
    assert r.yontem == "K-Medoids"
